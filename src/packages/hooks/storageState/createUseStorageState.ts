/**
 * 代码来自 ahooks 的 createUseStorageState
 * @link https://github.com/alibaba/hooks/blob/master/packages/hooks/src/createUseStorageState/index.ts#L19
 */
import { useState } from 'react';
import { useEventListener, useUpdateEffect, useMemoizedFn } from 'ahooks';
import { SafeLocalStorage } from '@sider/utils/safeStorage';
...
const SYNC_STORAGE_EVENT_NAME = 'AHOOKS_SYNC_STORAGE_EVENT_NAME';
...
export function createUseStorageState(getStorage: () => Storage | undefined) {
  function useStorageState<T>(key: string, options: Options<T> = {}) {
    ...
    const syncState = (event: StorageEvent) => {
      ...
      if (storage instanceof SafeLocalStorage) {
        // 对于 SafeLocalStorage，只接受 window.localStorage 的事
        if (
